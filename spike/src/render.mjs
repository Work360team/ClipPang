...
export async function buildVideoTrack(segments, workDir, opts, onProgress = () => {}) {
  ...
  for (const [i, seg] of segments.entries()) {
    ...
    await renderSegment(seg, path.join(dir, name), opts, workDir);
  ...
  return concatFiles("segments.txt", "video.mp4", workDir);
}
 ...
 */
export async function buildVoiceTrack(timeline, workDir) {
  ...
  await silenceWav(leadInMs, lead, workDir);
  await silenceWav(padMs, pad, workDir);
  await silenceWav(tailMs, tail, workDir);
  ...
  await concatFiles("voice.txt", "voice_raw.wav", workDir);
...
export async function burnAndMux(
  ...
  workDir,
  outFile,
  { bgm = null, bgmGainDb = -18, overlay = null } = {},
) {
  ...
  if (overlay) {
    ...
    filters.push("[0:v]ass=captions.ass,tpad=stop_mode=clone:stop_duration=1[v]");
  }
...
export async function poster(videoFile, outFile, workDir, atSec = 1.2) {
  await ffmpeg([
    "-ss", String(atSec),
    "-i", videoFile,
    "-frames:v", "1",
