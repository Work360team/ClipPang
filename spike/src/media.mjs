 ...
 */
export async function detectScenes(file, threshold = 0.32) {
...
async function edgeDensity(file, atSec, yFrac, hFrac = 0.2) {
  const { err } = await ffmpeg([
    "-ss", atSec.toFixed(2),
    "-i", file,
    "-frames:v", "1",
 ...
 */
export async function detectBurnedCaptions(file, meta) {
  ...
  for (const p of samples) {
    ...
    const band = await edgeDensity(file, at, 0.68);
    const control = await edgeDensity(file, at, 0.3);
 ...
 */
export async function renderSegment(seg, dst, { width, height, fps }, cwd) {
  const filters = [
    `fps=${fps}`,
...
export async function concatFiles(listFile, dst, cwd) {
  await ffmpeg(["-f", "concat", "-safe", "0", "-i", listFile, "-c", "copy", "-y", dst], { cwd });
  return dst;
}

/** สร้างไฟล์เงียบความยาวที่ต้องการ (24kHz mono 16-bit เท่ากับเสียงพากย์) */
export async function silenceWav(ms, dst, cwd) {
  await ffmpeg([
    "-f", "lavfi",
...
export async function toVoiceWav(src, dst, cwd) {
  await ffmpeg(["-i", src, "-ar", "24000", "-ac", "1", "-c:a", "pcm_s16le", "-y", dst], { cwd });
  return dst;
}
